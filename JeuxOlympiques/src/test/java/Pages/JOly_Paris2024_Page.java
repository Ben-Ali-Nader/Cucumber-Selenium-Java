...
public class JOly_Paris2024_Page {
	...
	@FindBy(xpath="//*[@title=\"Agrandir le bandeau de filtres\"]")
	WebElement filtre;

	@FindBy(xpath="//*[@id=\"react-aria6511280283-:rb:\"]")
	WebElement listePays;
	...
	public void medaillesparis2024() throws InterruptedException, IOException {
		...
		medailles.click();
		try {
			String headerTitle = Tableaudesmédailles.getText();  
			System.out.println("Le titre de la page est : " + headerTitle);
		} catch (Exception e) {
	...
	public void selectionnerPaysTunisie() throws InterruptedException {
	    try {filtre.click();
	        // Étape 1 : Cliquer sur le bouton pour ouvrir la liste
	        verifyElementVisibility(listePays, "le button est visible");
	    	listePays.click();
	        
	        // Étape 2 : Récupérer les options de la liste
	        List<WebElement> options = driver.findElements(By.xpath("//div[@role='listbox']//div[contains(@class, 'class-name-liste')]"));

	        // Étape 3 : Parcourir la liste et sélectionner "Tunisie"
	        for (WebElement option : options) {
	            if (option.getText().equals("Tunisie")) {
	                option.click(); // Cliquer sur "Tunisie"
	                System.out.println("Pays sélectionné : Tunisie");
	                break;
	            }
	        }
	    } catch (Exception e) {
	        System.err.println("Erreur lors de la sélection du pays : " + e.getMessage());
	    }
