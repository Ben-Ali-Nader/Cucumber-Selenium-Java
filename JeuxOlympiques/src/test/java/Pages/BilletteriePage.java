...
public class BilletteriePage {
    ...
    // Locators
    @FindBy(xpath = "//*[contains(text(),'Billetterie')]")
    ...
    public boolean setupDashboardBilletterie() {
        try {
            ...
            if (isElementClickable(btn_Billetterie)) {
        		...
        		js.executeScript("arguments[0].scrollIntoView({ behavior: 'smooth', block: 'center' });", btn_Billetterie);
    ...
    public void ImportantOfEventsBilleterie() {
        try {
            if (isElementVisible(ImportantInformation) && isElementVisible(ImportantInformationDescription)) {
                String info = ImportantInformation.getText();
                String descrip = ImportantInformationDescription.getText();
                
                System.out.println("Informations importantes : " + info);
                System.out.println("Description : " + descrip);
            } else {
                System.out.println("Pas d'informations à afficher à propos des billetteries.");
            }
        } catch (Exception e) {
            System.out.println("Erreur lors de l'accès aux informations importantes de la page Billetterie : " + e.getMessage());
        }
    }
